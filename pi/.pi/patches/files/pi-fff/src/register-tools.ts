...
// Match pi-collapse-tools rendering: hidden when collapsed, raw text when expanded.
function collapsedRenderResult(result: any, options: any, theme: any) {
...
export function registerTools(pi: ExtensionAPI, deps: ToolRegistrationDeps): void {
	...
	// `read` registration disabled — pi-collapse-tools owns the read tool wrapping.
	// fff autocomplete still resolves @paths before they reach read.
	...
	pi.registerTool({
		...
		renderResult: collapsedRenderResult,
	});
