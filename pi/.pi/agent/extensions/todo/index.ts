...
class TodoListComponent {
	...
	render(width: number): string[] {
		...
		if (this.todos.length === 0) {
			...
			const high = this.todos.filter((t) => t.priority === "high" && !t.done).length;
			let stats = `  ${th.fg("muted", `${done}/${total} completed`)}`;
			if (high > 0) stats += `  ${th.fg("error", `${high} high priority`)}`;
			...
			for (let i = 0; i < this.todos.length; i++) {
				...
				const pri = todo.priority === "high" ? th.fg("error", "▲") : todo.priority === "medium" ? th.fg("warning", "●") : th.fg("dim", "▽");
				const text = todo.done ? th.fg("dim", todo.text) : th.fg("text", todo.text);
				const prefix = selected ? th.fg("accent", "▸ ") : "  ";
				lines.push(truncateToWidth(`${prefix}${check} ${pri} ${id} ${text}`, width));
			}
		}

		lines.push("");
		lines.push(truncateToWidth(`  ${th.fg("dim", "↑/↓ navigate • Escape to close")}`, width));
		lines.push("");
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		promptSnippet: "Manage todos: list, add, toggle, remove, clear items with priority levels",
		promptGuidelines: [
			"Use this tool when the user asks to track tasks, create todo lists, or manage action items.",
		...
		async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
			switch (params.action) {
						...
						content: [
							{
								type: "text",
								text: todos.length
									? todos.map((t) => `[${t.done ? "x" : " "}] #${t.id} (${t.priority}): ${t.text}`).join("\n")
									: "No todos",
							},
						],
				...
				case "add": {
					...
					todos.push(newTodo);
					return {
						content: [{ type: "text", text: `Added todo #${newTodo.id} (${newTodo.priority}): ${newTodo.text}` }],
		...
		renderCall(args, theme, _context) {
			const pri = args.priority ? theme.fg("dim", ` [${args.priority}]`) : "";
			let text = theme.fg("toolTitle", theme.bold("todo ")) + theme.fg("muted", args.action) + pri;
			...
			switch (details.action) {
				case "list": {
					...
					for (const t of display) {
						...
						const pri = t.priority === "high" ? theme.fg("error", "▲") : t.priority === "medium" ? theme.fg("warning", "●") : theme.fg("dim", "▽");
						const itemText = t.done ? theme.fg("dim", t.text) : theme.fg("muted", t.text);
						listText += `\n${check} ${pri} ${theme.fg("accent", `#${t.id}`)} ${itemText}`;
					}
				...
				case "add": {
					...
					const pri = added.priority === "high" ? theme.fg("error", "▲") : added.priority === "medium" ? theme.fg("warning", "●") : theme.fg("dim", "▽");
					return new Text(
						theme.fg("success", "✓ Added ") + pri + " " +
