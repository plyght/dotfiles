...
| `MeanderingProgrammer/render-markdown.nvim` | Lightweight Markdown renderer with in-editor Unicode math rendering. |
| `nvim-neo-tree/neo-tree.nvim` | Optional file tree explorer (`:Neotree`). |
| `folke/trouble.nvim` | Diagnostics list UI (`:Trouble`). |
| `folke/snacks.nvim` | Utilities including the file explorer and inline PDF/image viewing in supported terminals. |
...
Markdown files render inline (`$...$`) and block (`$$...$$`) TeX formulas directly in Neovim. The renderer loads only for Markdown, R Markdown, and Quarto buffers, and uses the local `utftex` executable for lightweight Unicode output.

### PDF viewing

Open a PDF with Neovim to render its first page directly in the editor. This uses Snacks' built-in image viewer, Ghostty's Kitty Graphics Protocol support, and the installed ImageMagick and Poppler tools. PDF pages are converted on demand and cached locally.
